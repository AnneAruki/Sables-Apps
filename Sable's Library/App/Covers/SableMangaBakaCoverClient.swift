...
nonisolated struct SableMangaBakaStorefrontDiscovery: Sendable {
    ...
    struct OfficialPublisherSeriesPage: Sendable, Equatable {
        var url: URL
        var title: String
        var mediaType: String
    }
    ...
    func discover(
        ...
        async let yenPressReferences: [OfficialPublisherReference] =
            supplementalReferences(
            enabled: includesSupplementalSources
        ) {
            await self.yenPressPublisherReferences(
                for: series,
                languages: languages
            )
        }
        ...
        let yenPressPublisherReferences = (await yenPressReferences).filter {
            Self.providerIsEnabled(
                $0.provider,
                selectedProviders: selectedProviders
            )
        ...
        if !yenPressPublisherReferences.isEmpty {
            let pageCount = Set(
                yenPressPublisherReferences.map(\.pageURL)
            ).count
            let imprints = Set(
                yenPressPublisherReferences.compactMap(\.imprint)
            )
            .sorted()
            let imprintDetail = imprints.isEmpty
                ? ""
                : " (\(imprints.joined(separator: ", ")))"
            notes.append(
                "Yen family\(imprintDetail): checked \(pageCount) official volume page\(pageCount == 1 ? "" : "s") and found \(yenPressPublisherReferences.count) linked retailer cover\(yenPressPublisherReferences.count == 1 ? "" : "s")."
            )
        }
    ...
    private func yenPressPublisherReferences(
        for series: SableMangaBakaSeriesSummary,
        languages: Set<String>?
    ) async -> [OfficialPublisherReference] {
        let requestedLanguages = languages?.map(Self.normalizedLanguageTag)
        ...
        guard requestedLanguages == nil
            || requestedLanguages?.contains("en") == true,
              expectedMediaType == "manga"
                || expectedMediaType == "novel" else {
            return []
        }

        var seriesPages: [OfficialPublisherSeriesPage] = []
        for title in Self.sevenSeasRequestedTitles(for: series).prefix(4) {
            guard let searchJSON = await Self.yenPressSearchJSON(
                query: title
            ) else {
                continue
            }
            seriesPages = Self.yenPressSeriesPages(
                in: searchJSON,
                series: series
            )
            if !seriesPages.isEmpty {
                break
            }
        }
        ...
        for seriesPage in seriesPages {
            guard !Task.isCancelled,
                  let seriesHTML = await Self.storefrontPageHTML(
                    from: seriesPage.url.absoluteString
                  ) else {
                continue
            }
            let pageURLs = Self.yenPressVolumePageURLs(
                in: seriesHTML,
                seriesTitle: seriesPage.title,
                mediaType: seriesPage.mediaType
            )
            references.append(
                contentsOf: await officialPublisherReferences(
                    pageURLs: pageURLs,
                    referer: seriesPage.url
                ) { html, pageURL in
                    Self.yenPressPublisherReferences(
                        in: html,
                        pageURL: pageURL,
                        mediaType: seriesPage.mediaType
                    )
    ...
    private static func yenPressSearchJSON(
        query: String
    ) async -> String? {
        let rawToken = SableLibraryUserSettings()
            .loadProviderCredentials()
            .yenPressSearchToken
            .trimmingCharacters(in: .whitespacesAndNewlines)
        guard !rawToken.isEmpty else { return nil }
        let authorization = rawToken.lowercased().hasPrefix("bearer ")
            ? rawToken
            : "Bearer \(rawToken)"
        guard let url = URL(
            string:
                "https://enterprise-search.yenpress.com/api/as/v1/engines/yenpress/search"
        ) else {
            return nil
        }
        let body: [String: Any] = [
        ...
        guard let response = try? await storefrontPageSession.data(
            for: request
        ),
              let http = response.1 as? HTTPURLResponse,
              (200..<300).contains(http.statusCode),
              !response.0.isEmpty,
              response.0.count <= 2_000_000 else {
            return nil
        }
        return String(data: response.0, encoding: .utf8)
    }

    static func yenPressSeriesPages(
        in searchJSON: String,
        series: SableMangaBakaSeriesSummary
    ) -> [OfficialPublisherSeriesPage] {
        guard let data = searchJSON.data(using: .utf8),
        ...
        let expectedMediaType = SableLibraryCoverDownloadPlanner
            .preferredProviderBookTypeForDownload(mediaType: series.type)
        let requestedTypes: Set<String>
        if expectedMediaType == "novel" {
            requestedTypes = ["novel", "audiobook"]
        } else if let expectedMediaType {
            requestedTypes = [expectedMediaType]
        } else {
            requestedTypes = []
        }
        let requestedTitles = Self.sevenSeasRequestedTitles(for: series)
        var pages: [OfficialPublisherSeriesPage] = []
        var seen = Set<String>()

        for row in rows {
            guard let rawTitle = (row["title"] as? [String: Any])?["raw"]
                    as? String,
                  let rawPath = (row["url"] as? [String: Any])?["raw"]
                    as? String,
                  rawPath.hasPrefix("/series/"),
                  let mediaType = yenPressMediaType(in: rawTitle)
                    ?? expectedMediaType,
                  requestedTypes.contains(mediaType),
                  requestedTitles.contains(where: {
                    normalizedPublisherCatalogTitle(
                        officialPublisherSeriesTitle(from: $0)
                    )
                        == normalizedPublisherCatalogTitle(
                            officialPublisherSeriesTitle(
                                from: rawTitle
                            )
                        )
                  }),
                  let url = URL(
                    string: rawPath,
                    relativeTo: URL(string: "https://yenpress.com")
                  )?.absoluteURL,
                  seen.insert(url.absoluteString).inserted else {
                continue
            }
        ...
        return pages.sorted {
            let lhsAudio = $0.mediaType == "audiobook"
            let rhsAudio = $1.mediaType == "audiobook"
            if lhsAudio != rhsAudio {
                return !lhsAudio
            }
    ...
    static func yenPressVolumePageURLs(
        in seriesHTML: String,
        seriesTitle: String,
        mediaType: String
    ) -> [URL] {
        let pattern =
            #"(?is)<a[^>]+href\s*=\s*["'](/titles/[^"']+)["'][^>]*>(.*?)</a>"#
        guard let regex = try? NSRegularExpression(pattern: pattern) else {
            return []
        }
        let baseTitle = officialPublisherSeriesTitle(from: seriesTitle)
        ...
        where match.numberOfRanges > 2 {
            ...
            let body = String(seriesHTML[bodyRange])
            let linkedTitle = firstPathCapture(
                in: body,
                pattern: #"(?is)alt\s*=\s*["']([^"']+)["']"#
            )
            .map(decodedPublisherHTML)
            ...
            guard SableLibraryCoverDownloadPlanner.providerTitle(
                ...
                belongsTo: baseTitle
                  ...
                  let volumeNumber = SableLibraryCoverDownloadPlanner
                    .explicitVolumeNumber(in: linkedTitle) else {
                continue
            }
            if let detectedType = yenPressMediaType(in: linkedTitle),
               detectedType != mediaType,
               mediaType != "audiobook" {
                continue
            }
            let rawPath = decodedPublisherHTML(
                String(seriesHTML[pathRange])
            )
            guard let url = URL(
                string: rawPath,
                relativeTo: URL(string: "https://yenpress.com")
    ...
    static func yenPressPublisherReferences(
        in pageHTML: String,
        pageURL: URL,
        mediaType: String
    ) -> [OfficialPublisherReference] {
        ...
        .trimmingCharacters(in: .whitespacesAndNewlines),
              ...
              let rawImageURL = firstPathCapture(
                in: pageHTML,
                pattern:
                    #"(https://images\.yenpress\.com/imgs/[0-9Xx]+\.jpg\?w=408&amp;h=[0-9]+[^"'\s<]*)"#
              ) ?? firstPathCapture(
                in: pageHTML,
                pattern:
                    #"(https://images\.yenpress\.com/imgs/[0-9Xx]+\.jpg\?w=408&h=[0-9]+[^"'\s<]*)"#
        ...
        let imprint = firstPathCapture(
            in: pageHTML,
            pattern:
                #"(?is)>\s*Imprint\s*</span>\s*<p[^>]*class\s*=\s*["'][^"']*info[^"']*["'][^>]*>(.*?)</p>"#
        )
        .map(decodedPublisherHTML)?
        .trimmingCharacters(in: .whitespacesAndNewlines)
        let isAudiobook = mediaType == "audiobook"
        ...
        where match.numberOfRanges > 2 {
            ...
            let label = decodedPublisherHTML(
                String(pageHTML[bodyRange])
            )
            let rawURL = decodedPublisherHTML(
                String(pageHTML[urlRange])
            )
            guard let storeURL = URL(string: rawURL),
                  let provider = sevenSeasRetailerProvider(
                    label: label,
                    url: storeURL,
                    isAudiobook: isAudiobook
    ...
    private static func yenPressMediaType(
        in value: String
    ) -> String? {
