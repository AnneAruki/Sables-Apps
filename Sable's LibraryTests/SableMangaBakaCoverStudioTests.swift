...
final class SableMangaBakaCoverStudioTests: XCTestCase {
    ...
    func testYenSearchSeparatesNovelMangaAndAudioSeries() {
        let series = SableMangaBakaSeriesSummary(
            id: 86,
            title: "86--EIGHTY-SIX",
            nativeTitle: nil,
        ...
        let json = """
        {
          "results": [
            {
              "title": {"raw": "86--EIGHTY-SIX (manga)"},
              "url": {"raw": "/series/86-eighty-six-manga"}
            },
            {
              "title": {"raw": "86--EIGHTY-SIX (audio)"},
              "url": {"raw": "/series/86-eighty-six-audio"}
            },
            {
              "title": {"raw": "86--EIGHTY-SIX (light novel)"},
              "url": {"raw": "/series/86-eighty-six-light-novel"}
            },
            {
              "title": {"raw": "86--EIGHTY-SIX Alter (light novel)"},
              "url": {"raw": "/series/86-eighty-six-alter-light-novel"}
            }
          ]
        }
        """
        ...
        let pages = SableMangaBakaStorefrontDiscovery
            .yenPressSeriesPages(in: json, series: series)
        ...
        XCTAssertEqual(
            pages.map(\.url.path),
            [
                "/series/86-eighty-six-light-novel",
                "/series/86-eighty-six-audio"
            ]
        )
    }

    func testYenSeriesPageKeepsOnlyMatchingVolumes() {
        let html = """
        <a href="/titles/9780000000000-other-story-vol-1">
          <img alt="Other Story, Vol. 1 (light novel)">
        </a>
        <a href="/titles/9781975303129-86-eighty-six-vol-1-light-novel">
          <img alt="86--EIGHTY-SIX, Vol. 1 (light novel)">
        </a>
        <a href="/titles/9781975303143-86-eighty-six-vol-2-light-novel">
          <span>86--EIGHTY-SIX, Vol. 2 (light novel)</span>
        </a>
        """

        let urls = SableMangaBakaStorefrontDiscovery
            .yenPressVolumePageURLs(
                in: html,
                seriesTitle: "86--EIGHTY-SIX (light novel)",
                mediaType: "novel"
            )
        ...
        XCTAssertEqual(
            urls.map(\.path),
            [
                "/titles/9781975303129-86-eighty-six-vol-1-light-novel",
                "/titles/9781975303143-86-eighty-six-vol-2-light-novel"
            ]
        )
    }

    func testYenSearchLetsExactUntypedSeriesReachVolumeTypeCheck() {
        let series = SableMangaBakaSeriesSummary(
            id: 202,
            title: "Black Butler",
            nativeTitle: nil,
        ...
        let json = """
        {
          "results": [
            {
              "title": {"raw": "Black Butler"},
              "url": {"raw": "/series/black-butler"}
            },
            {
              "title": {"raw": "Black Butler Serial"},
              "url": {"raw": "/series/black-butler-serial"}
            }
          ]
        }
        """
        ...
        XCTAssertEqual(pages.first?.url.path, "/series/black-butler")
        XCTAssertEqual(pages.first?.mediaType, "manga")
    }

    func testYenTitlePageUsesOfficialCoverAndSupportedRetailers()
        throws {
        let pageURL = try XCTUnwrap(
            URL(
                string:
                    "https://yenpress.com/titles/9781975303129-86-eighty-six-vol-1-light-novel"
            )
        )
        let html = """
        <h1>86--EIGHTY-SIX, Vol. 1 (light novel)</h1>
        <img src="https://images.yenpress.com/imgs/9781975303129.jpg?w=408&amp;h=612&amp;type=books&amp;s=abc">
        <span class="type">Imprint</span>
        <p class="info">Yen On</p>
        <a href="https://www.amazon.com/s/?keywords=9781975303136">
          Amazon
        </a>
        <a href="https://global.bookwalker.jp/search/?word=86">
          Book Walker
        </a>
        <a href="https://store.kobobooks.com/search?Query=86">
          Kobo
        </a>
        <a href="https://www.barnesandnoble.com/s/86">
          NOOK
        </a>
        """
        ...
        XCTAssertTrue(
            references.allSatisfy {
                $0.imageURL.contains(
                    "images.yenpress.com/imgs/9781975303129.jpg"
                )
            }
        )
    }
